...
class TimeCard:
    def __init__(
        self, date: Optional[date] = None, entries: List[TimeCardEntry] = []
    ) -> None:
        self.date = date
...
class TimeCardDatabase:
    ...
    def find_records(
        self, text: str, date1: date = date(2019, 1, 1), date2: date = date.today()
    ) -> List[TimeCardEntry]:
        """
...
if __name__ == "__main__":
    today = date.today()
    db = TimeCardDatabase()
    tc = TimeCardEntry(today, 0, "000020", "039", 8, "LEAD WORK", "OVERHEAD", "R")
